 ...
 * 
 * <p>
 * The elevator's behavior is impacted by the following parameters:
 * <ul>
 * <i>Elevator capacity.</i> The maximum number of passengers that can fit on an
 * elevator.
 * </ul>
 * <ul>
 * <i>Elevator speed.</i> The maximum speed an elevator can travel at in
 * feet/sec.
 * It is assumed to be the same for all elevators.
 * </ul>
 * <ul>
 * <i>Elevator acceleration.</i> The rate at which the elevator can increase or
 * decrease
 * speed in ft/sec2. It is assumed to be the same for all elevators. The higher
 * the acceleration,
 * the faster the elevator can accelerate and decelerate, allowing its average
 * speed to be faster.
 * </ul>
 * <ul>
 * <i>Number of floors.</i> The number of floors in the building, including the
 * ground floor. It is
 ...
 * </ul>
 * <ul>
 * <i>Floor height.</i> The height of the floors in feet. It is assumed that
 * each floor is
 * the same height.
 * </ul>
 * <ul>
 * <i>Number of elevators.</i> The number of elevators in the building. Elevator
 * numbering starts
 ...
 * <ul>
 * <i>Door opening & closing.</i> Elevator doors automatically open when
 * arriving at a floor and
 * close before departing. When passengers are waiting to board an elevator, the
 * doors will
 * stay open until all passengers are boarded. Each open and close action takes
 * 1 second.
 * </ul>
 * <ul>
 * <i>Elevator buttons.</i> Arriving passengers press the appropriate elevator
 * button and the
 * light is automatically turned off when the elevator arrives at the associated
 * floor.
 * </ul>
 * <ul>
 * <i>Elevator speed and acceleration.</i> The elevator will automatically
 * accelerate to the maximum
 * speed possible until there is only enough time left to slow down to arrive at
 * the target floor.
 * </ul>
 * 
 * <p>
 * There is substantial elevator status information available:
 * <ul>
 * <i>Elevator speed.</i> The current speed of the elevator. Positive speed
 * indicates an elevator
 * heading up while negative indicates an elevator going down.
 * </ul>
 * <ul>
 * <i>Elevator acceleration.</i> The acceleration of the elevators.
 * </ul>
 * <ul>
 * <i>Elevator button status.</i> For each elevator, which floor buttons on the
 * elevator have
 * been pressed.
 * </ul>
 * <ul>
 * <i>Elevator position.</i> The current position of the elevator, both in feet
 * and to
 * the closest floor. Position is measured in feet above ground floor with zero
 * being the bottom.
 * </ul>
 * <ul>
 * <i>Elevator door status.</i> Whether the doors for an elevator are open or
 * closed. The
 * status may also indicate a transition between open or closed.
 * </ul>
 * <ul>
 ...
 * </ul>
 * <ul>
 * <i>Elevator committed direction.</i> The current committed direction of the
 * elevator. Elevators
 * responding to a passenger floor button must have a committed direction, up or
 * down. Otherwise,
 * passengers wishing to go up would board the same elevators as passengers
 * wishing to go down.
 * Accordingly, each elevator can have an up, down or uncommitted committed
 * direction set in response
 * to passenger travel..
 * </ul>
 * <ul>
 * <i>Elevator floor service.</i> Whether a particular elevator services a
 * particular floor. When
 * elevators are allowed to only service certain floors, this can help to
 * achieve greater passenger
 * service. Every elevator must service the ground floor.
 * </ul>
 * <ul>
 * <i>Elevator weight.</i> This provides the current weight of the elevator less
 * the weight of the
 * empty elevator - hence the weight of the passengers on board. This can be
 * useful for detecting
 * when the elevator is getting full.
 * </ul>
 * 
 * <p>
 * The elevators can be controlled by setting the following parameters:
 * <ul>
 * <i>Elevator target.</i> Sets the floor target of a particular elevator.
 * Typically, this target
 * is set in response to a given passenger selecting a floor up or down button,
 * or selecting a floor
 * button in an elevator. The elevator will travel to that target and stop until
 * directed to the
 * next target.
 * </ul>
 * <ul>
 * <i>Elevator committed direction.</i> Sets the current committed direction of
 * the elevator. This
 ...
 * elevator may be
 * going down, for example, but its committed direction is up - reflecting the
 * fact it has been
 * dispatched down to pick up a passenger who is traveling up. Whenever an
 * elevator is being
 * dispatched to a target floor, it should have a committed direction set.
 * </ul>
 * <ul>
 * <i>Elevator floor service.</i> Sets whether a particular elevator services a
 * particular floor.
 * In larger buildings, it is typically better to set elevator service to a
 * particular subset of floors.
...
public interface IElevator extends java.rmi.Remote {
	...
	/**
	 * Retrieves the committed direction of the specified elevator (up / down /
	 * uncommitted).
	 * 
	 * @param elevatorNumber - elevator number whose committed direction is being
	 *                       retrieved
	 * @return the current direction of the specified elevator where up=0, down=1
	 *         and uncommitted=2
	...
	/**
	 * Provides the current acceleration of the specified elevator in feet per
	 * sec^2.
	 * 
	 * @param elevatorNumber - elevator number whose acceleration is being retrieved
	 * @return returns the acceleration of the indicated elevator where positive
	 *         speed is acceleration and negative is deceleration
	 * @throws RemoteException RMI Invalid exception
	 */
	public int getElevatorAccel(int elevatorNumber) throws java.rmi.RemoteException;

	/**
	 * Provides the status of a floor request button on a specified elevator
	 * (on/off).
	 * 
	 ...
	 *                       elevator
	 * @return returns boolean to indicate if floor button on the elevator is active
	 *         (true) or not (false)
	 * @throws RemoteException RMI Invalid exception
	 */
	...
	/**
	 * Provides the current status of the doors of the specified elevator
	 * (open/closed).
	 * 
	 * @param elevatorNumber - elevator number whose door status is being retrieved
	 * @return returns the door status of the indicated elevator where 1=open and
	 *         2=closed
	 * @throws RemoteException RMI Invalid exception
	 */
	 ...
	 * @return returns the floor number of the floor closest to the indicated
	 *         elevator
	 * @throws RemoteException RMI Invalid exception
	 */
	...
	/**
	 * Provides the current location of the specified elevator in feet from the
	 * bottom of the building.
	 * 
	 * @param elevatorNumber - elevator number whose location is being retrieved
	 * @return returns the location in feet of the indicated elevator from the
	 *         bottom of the building
	 * @throws RemoteException RMI Invalid exception
	 */
	 ...
	 * @return returns the speed of the indicated elevator where positive speed is
	...
	/**
	 * Retrieves the maximum number of passengers that can fit on the specified
	 * elevator.
	 * 
	 * @param elevatorNumber - elevator number whose service is being retrieved
	...
	/**
	 * Retrieves whether or not the specified elevator will service the specified
	 * floor (yes/no).
	 * 
	 * @param elevatorNumber elevator number whose service is being retrieved
	 * @param floor          floor whose service status by the specified elevator is
	 *                       being retrieved
	 * @return service status whether the floor is serviced by the specified
	 *         elevator (yes=true,no=false)
	 * @throws RemoteException RMI Invalid exception
	 */
	...
	/**
	 * Sets whether or not the specified elevator will service the specified floor
	 * (yes/no).
	 * 
